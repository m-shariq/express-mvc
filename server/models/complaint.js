...
module.exports = class complaint {
  ...
  static getResolvedWorkerComplaint() {
    return db.execute(
      `SELECT * FROM complaint,worker,city,province,complaint_report WHERE complaint.worker_id=worker.worker_id and complaint.compaint_id=complaint_report.complaint_id and worker.city_id=city.city_id and worker.province_id=province.province_id and compaint_status=?`,
      [0]
    );
  }
  static getResolvedCitizenComplaint() {
    return db.execute(
      `SELECT * FROM complaint,citizen,city,province,complaint_report WHERE complaint.citizen_id=citizen.citizen_id and complaint.compaint_id=complaint_report.complaint_id and citizen.city_id=city.city_id and citizen.province_id=province.province_id and compaint_status=?`,
      [0]
    );
  }
